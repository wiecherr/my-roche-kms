  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || "Error creating the customer");
      }

      const result = await res.json();
      setMessage({
        type: "success",
        text: `Customer ${result.customer_name || customerName} successfully created!`,
      });
    ...
    <div className="w-full max-w-3xl mx-auto space-y-6 py-2">
      ...
      <div className="w-full border-b border-slate-800 pb-6">
        ...
        >
          ...
          <span>Back to Customers Overview</span>
        </Link>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-wide">Create New Customer</h1>
            <p className="text-xs text-slate-400">
              Register a customer organization and automatically issue an associated signature key pair
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="space-y-2">
          <label htmlFor="customerName" className="text-xs font-semibold uppercase tracking-wider text-slate-300 flex items-center gap-1.5">
          ...
          <input
            ...
            placeholder="e.g. Lab Ocean Hospital"
        ...
        <div className="space-y-2">
          <label htmlFor="customerEmail" className="text-xs font-semibold uppercase tracking-wider text-slate-300 flex items-center gap-1.5">
          ...
          <p className="text-[11px] text-slate-500">
            The target email address where key delivery packages and notification manifests will be sent.
          </p>
        </div>
