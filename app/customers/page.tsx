...
interface CustomerItem {
  ...
  customer_sign_pem_privkey: string;
  customer_sign_pem_pubkey: string;
  created_at: string;
}

// Interactive component for compact PEM key display
function PemKeyViewer({ label, pemKey, isPrivate }: { label: string; pemKey: string; isPrivate?: boolean }) {
  ...
  // Creates a clean 2-line preview (header and footer of the PEM string)
...
export default function CustomersPage() {
  ...
  useEffect(() => {
    ...
    async function fetchCustomers() {
      try {
        const response = await fetch("http://localhost:3010/customers", { cache: "no-store" });
        if (!response.ok) throw new Error(`Error: ${response.statusText}`);
        const data = await response.json();
        
        if (isMounted) {
          setCustomeritems(Array.isArray(data) ? data : []);
        }
      } catch (error) {
    ...
    <div className="w-full space-y-6 py-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Overview of all registered customer organizations and their signing key pairs
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-xs font-mono font-medium text-slate-300">
            ...
            Count: <span className="text-white font-bold">{customeritems.length}</span>
          </span>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex h-64 flex-col items-center justify-center gap-3 text-slate-400">
          ...
          <p className="text-xs font-mono">Loading customer organizations from database...</p>
        </div>
        ...
        <div className="rounded-2xl border border-dashed border-slate-800 bg-slate-900/40 p-12 text-center text-slate-500">
          ...
          <p className="text-sm font-medium">No customers found in the database.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {customeritems.slice(0, 50).map((item) => (
            <div
              key={item.id}
            ...
            >
              ...
              {(item.customer_sign_pem_privkey || item.customer_sign_pem_pubkey) && (
                <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-800/80 space-y-3">
                  {item.customer_sign_pem_privkey && (
                    <PemKeyViewer 
                      label="PEM Buffer for Sign Private Key" 
                      pemKey={item.customer_sign_pem_privkey} 
                      isPrivate={true} 
                    />
                  )}

                  {item.customer_sign_pem_pubkey && (
                    <PemKeyViewer 
                      label="PEM Buffer for Sign Public Key" 
                      pemKey={item.customer_sign_pem_pubkey} 
                      isPrivate={false} 
                    />
                  )}
                </div>
